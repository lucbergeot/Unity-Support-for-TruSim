...
namespace Convai.Scripts.Runtime.Features
{
    ...
    public class ConvaiActionsHandler : MonoBehaviour
    {
        ...
        private void OnActionStarted(string actionName, GameObject target)
        {
            ...
            Debug.Log($"Action started: {actionName} on {target.name}");
        }

        // Mark that an action has ended
        private void OnActionEnded(string actionName, GameObject target)
        {
            isActionInProgress = false; // Set to false when the action finishes
            Debug.Log($"Action ended: {actionName} on {target.name}");
        }

        // Expose the state of action for external checking
        public bool IsActionInProgress()
        {
            return isActionInProgress;  // Return true if any action is in progress
        }
...
private void Update()
{
    ...
    {
        string actionResponse = actionResponseList[0];
        actionResponseList.RemoveAt(0);

        if (!string.IsNullOrEmpty(actionResponse))
        {
            ParseActions(actionResponse);
        }
        else
        {
            ConvaiLogger.DebugLog("Empty action response received, skipping parsing.", ConvaiLogger.LogCategory.Actions);
        }
    }
}
...
private void ParseActions(string actionsString)
{
    ...
    {
        ConvaiLogger.DebugLog("No actions to parse. Received an empty action string.", ConvaiLogger.LogCategory.Actions);
        return;
    }
...
if (matchingAction == null || !string.Equals(matchingAction.action, actionString, StringComparison.OrdinalIgnoreCase))continue;
