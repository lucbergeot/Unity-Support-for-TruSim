...
public async void SendTextDataNPC2NPC(string message)
{
    ...
    try
    {
        ...
        // Ensure the NPC is marked as in conversation and active
        IsInConversationWithAnotherNPC = true;
        ConvaiNPC.isCharacterActive = true;
        ConvaiLogger.DebugLog($"NPC {CharacterName} is responding with message: {message}", ConvaiLogger.LogCategory.Character);
    }
    catch (Exception ex)
    {
