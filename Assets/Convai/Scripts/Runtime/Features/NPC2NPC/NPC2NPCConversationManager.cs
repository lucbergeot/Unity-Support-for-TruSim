...
private void Start()
{
    ...
    SetNPCInactiveForPlayer("Stella Nebulina");
    StartConversationWithAllNPCs();
}

private void Update()
{
    // Find Dr. Quack Matterson and Stella Nebulina controllers
    ConvaiGroupNPCController drQuack = FindNPCByName("Convai NPC Dr. Quack Matterson");
    ConvaiGroupNPCController stella = FindNPCByName("Convai NPC Stella Nebulina");

    if (drQuack == null || stella == null)
        return;

    // Define proximity radius
    float proximityRadius = 5.0f; // Adjust the value as needed

    // Check if the distance between Dr. Quack and Stella is within the proximity radius
    if (Vector3.Distance(drQuack.transform.position, stella.transform.position) <= proximityRadius)
    {
        // Start conversation if they are near each other
        if (!drQuack.IsInConversationWithAnotherNPC && !stella.IsInConversationWithAnotherNPC)
        {
            StartNPCConversation(drQuack, stella);
        }
    }
}
public void TriggerConversationWithStella()
{
    // Find Dr. Quack Matterson and Stella Nebulina controllers
    ConvaiGroupNPCController drQuack = FindNPCByName("Convai NPC Dr. Quack Matterson");
    ConvaiGroupNPCController stella = FindNPCByName("Convai NPC Stella Nebulina");

    if (drQuack != null && stella != null && !drQuack.IsInConversationWithAnotherNPC && !stella.IsInConversationWithAnotherNPC)
    {
        StartNPCConversation(drQuack, stella);
    }
    else
    {
        ConvaiLogger.Warn("Unable to start conversation: Either one of the NPCs is null or already in conversation.", ConvaiLogger.LogCategory.Character);
    }
}

// Utility method to find an NPC by name
private ConvaiGroupNPCController FindNPCByName(string npcName)
{
    return npcGroups
        .SelectMany(group => new List<ConvaiGroupNPCController> { group.GroupNPC1, group.GroupNPC2 })
        .FirstOrDefault(npc => npc.CharacterName == npcName);
}

private void StartNPCConversation(ConvaiGroupNPCController npc1, ConvaiGroupNPCController npc2)
{
    NPCGroup npcGroup = npcGroups.Find(group => group.BelongToGroup(npc1) && group.BelongToGroup(npc2));
    if (npcGroup != null)
    {
        // Ensure both NPCs are activated before starting the conversation
        npc1.ConvaiNPC.isCharacterActive = true;
        npc2.ConvaiNPC.isCharacterActive = true;

        InitializeNPCGroup(npcGroup);
        ConvaiLogger.DebugLog($"Starting conversation between {npc1.CharacterName} and {npc2.CharacterName}", ConvaiLogger.LogCategory.Character);

        // Make NPCs look at each other
        npc1.transform.LookAt(npc2.transform.position);
        npc2.transform.LookAt(npc1.transform.position);
    }
}

private void SetNPCInactiveForPlayer(string npcName)
{
    ConvaiGroupNPCController npcController = npcGroups
        .SelectMany(group => new List<ConvaiGroupNPCController> { group.GroupNPC1, group.GroupNPC2 })
        .FirstOrDefault(npc => npc.CharacterName == npcName);

    if (npcController != null)
    {
        npcController.ConvaiNPC.isCharacterActive = false;
    }
}
...
private IEnumerator RelayMessageCoroutine(string message, ConvaiGroupNPCController sender, NPCGroup npcGroup)
{
    ...
    // Ensure the receiver is ready to talk
    ...
    try
    {
        ...
        // Send the message directly to the receiving NPC without displaying in player's UI
        receiver.SendTextDataNPC2NPC(processedMessage);

        // Update the receiver's speech bubble or UI for NPC-specific interaction
        receiver.AttachSpeechBubble(); // Ensure the response is displayed in NPC-specific UI
        ConvaiLogger.DebugLog($"Message directed to {receiver.CharacterName}: {processedMessage}", ConvaiLogger.LogCategory.Character);

        // Trigger speaker switch after sending message
        SwitchSpeaker(sender);

        // Ensure the receiver gets activated to speak
        receiver.IsInConversationWithAnotherNPC = true;
        ConvaiLogger.DebugLog($"{receiver.CharacterName} is now set to respond.", ConvaiLogger.LogCategory.Character);
    }
    catch (Exception e)
    {
