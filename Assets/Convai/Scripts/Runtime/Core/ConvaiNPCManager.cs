...
namespace Convai.Scripts.Runtime.Core
{
    ...
    public class ConvaiNPCManager : MonoBehaviour
    {
        ...
        private float visionConeAngle = 360f;
...
private void LateUpdate()
{
    if (_mainCamera == null)
    {
        ...
        if (_mainCamera == null)
        {
            Debug.LogError("Main camera is not assigned or cannot be found. Please ensure there is a main camera in the scene.");
            return;
        }
    }

    float detectionRadius = rayLength; // Use the rayLength variable to define the detection radius
    Collider[] hitColliders = Physics.OverlapSphere(_mainCamera.transform.position, detectionRadius);

    bool foundConvaiNPC = false;

    foreach (Collider hitCollider in hitColliders)
    {
        // Get the Convai NPC script from the hit object
        nearbyNPC = GetConvaiNPC(hitCollider.gameObject);

        if (nearbyNPC != null && nearbyNPC.name == "Convai NPC Dr. Quack Matterson") // Check if it's the specific NPC
        {
            foundConvaiNPC = true;

            // Check if this NPC is already activated and in a conversation or if it's a new NPC
            if (_lastHitNpc != nearbyNPC && !CheckForNPCToNPCConversation(nearbyNPC))
            {
                UpdateActiveNPC(nearbyNPC); // Activate only the desired NPC
            }
        }
    }

    // Commented out the deactivation logic to ensure Dr. Quack Matterson remains active
    /*
    if (!foundConvaiNPC && _lastHitNpc != null)
    {
        UpdateActiveNPC(null); // This will no longer deactivate the NPC when not found
    }
    */
}
