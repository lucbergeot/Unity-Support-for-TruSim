...
namespace Convai.Scripts.Runtime.Core
{
    ...
    public class ConvaiNPC : MonoBehaviour
    {
        ...
        public bool IsPerformingAction()
        {
            if (actionsHandler != null)
            {
                return actionsHandler.IsActionInProgress(); 
            }
            return false;
        }
        ...
        public async void SendTextDataAsync(string text)
{
    try
    {
        ...
        if (text.StartsWith("$"))
        {
            // Remove the special character and set the flag
            text = text.Substring(1);
            disableAudio = true;
        }
        await ConvaiGRPCAPI.Instance.SendTextData(_client, text, characterID,
            _isActionActive, _isLipSyncActive, ActionConfig, FaceModel, SpeakerID, this, disableAudio);
    }
    catch (Exception ex)
    {
